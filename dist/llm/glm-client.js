 ...
 */
const LEVELS_FLASH = ["low", "high", "max"];
...
export function getAvailableModels() {
    ...
    if (!override)
        return BUILTIN_AVAILABLE_MODELS;
    const ids = override
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
...
export class GlmClient {
    ...
    constructor() {
        ...
        if (!apiKey) {
            throw new Error("No API key found. Set the Z_AI_API_KEY environment variable, or run `glm-acp-agent --setup` to store one.");
        }
    ...
    async *streamChat(messages, signal, options) {
        ...
        for await (const chunk of stream) {
            ...
            if (rawUsage) {
                ...
                if (typeof rawUsage.completion_tokens_details?.reasoning_tokens === "number") {
