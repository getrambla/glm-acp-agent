 ...
 */
export function syncManifestContent(content, pkg) {
    const manifest = parseManifest(content);
    if (typeof manifest.version !== "string") {
        throw new Error(`"version" field not found (or not a string) in ${MANIFEST_PATH}`);
    }
    const npx = manifest.distribution?.npx;
    if (!npx ||
        typeof npx.package !== "string" ||
        !npx.package.startsWith(`${pkg.name}@`)) {
        throw new Error(`distribution.npx.package pin not found (or not targeting ${pkg.name}) in ${MANIFEST_PATH}`);
    }
...
function parseManifest(content) {
    try {
        ...
        throw new Error(`${MANIFEST_PATH} could not be parsed: ${error.message}`, { cause: error });
    }
}
const describeValue = (value) => typeof value === "string" ? value : "(missing)";
/**
...
function main(argv) {
    ...
    if (check) {
        ...
        console.error(`  version:        ${describeValue(manifest.version)} (expected ${pkg.version})`);
        console.error(`  npx package:    ${describeValue(pin)} (expected ${pkg.name}@${pkg.version})`);
        console.error(`Fix: run \`npm run build && node dist/registry/sync-manifest.js\` and commit the result.`);
        return 1;
    }
    ...
    if (staged.status !== 0) {
        console.error(`synced ${MANIFEST_PATH} but \`git add\` failed — stage it manually`);
        return 1;
    }
    return 0;
}
