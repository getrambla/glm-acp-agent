 ...
 * precedence order.
 */
export function discoverSlashCommands(cwd) {
    const found = new Map();
 ...
 */
function collectCommandFiles(dir, prefix, found) {
    if (prefix.length >= MAX_COMMAND_DEPTH || found.size >= MAX_COMMANDS)
        return;
    for (const entry of readDirSafe(dir)) {
        if (found.size >= MAX_COMMANDS)
            return;
...
function addCommand(found, name, path) {
    if (!COMMAND_NAME_PATTERN.test(name) || found.has(name))
        return;
 ...
 */
export function parseSlashCommand(text, commands) {
 ...
 */
export function renderSlashCommand(parsed) {
