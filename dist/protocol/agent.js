...
import { GlmClient, getAvailableModels, getDefaultModel, getContextWindow, isVisionNativeModel, ERR_CONTEXT_OVERFLOW, getThoughtLevels, resolveThoughtLevel, isThoughtLevel, } from "../llm/glm-client.js";
...
const SESSION_MODE_IDS = SESSION_MODES.map((mode) => mode.id);
function isSessionModeId(value) {
    return typeof value === "string" && SESSION_MODE_IDS.includes(value);
}
/**
 ...
 */
function envMaxTurns() {
    const raw = process.env["ACP_GLM_MAX_TURNS"];
    if (raw === undefined || raw === "")
        return undefined;
    const parsed = Number(raw);
    if (!Number.isFinite(parsed) || Math.floor(parsed) < 1) {
        process.stderr.write(`glm-acp-agent: ignoring invalid ACP_GLM_MAX_TURNS="${raw}"\n`);
        return undefined;
    }
    return Math.floor(parsed);
}
export class GlmAcpAgent {
    connection;
    sessions = new Map();
    ...
    async initialize(params) {
        ...
        const negotiatedVersion = params.protocolVersion <= VERSION ? params.protocolVersion : VERSION;
     ...
     */
    async applySessionModel(sessionId, session, modelId) {
        const available = getAvailableModels();
        const known = available.find((m) => m.modelId === modelId);
        if (!known) {
            process.stderr.write(`[glm-acp-agent] warning: model "${modelId}" is not in the advertised list; using as-is.\n`);
        }
        ...
        await safeSessionUpdate(this.connection, {
            ...
            update: {
                ...
                configOptions: this.configOptionsState(session.model, session.thoughtLevel, session.mode),
            },
        });
    }
    /**
     ...
     */
    configOptionsState(model, thoughtLevel, currentMode) {
    ...
    async setSessionMode(params) {
        ...
        if (!isSessionModeId(params.modeId)) {
            throw new Error(`Invalid modeId: ${params.modeId}. Valid modes are: ${SESSION_MODE_IDS.join(", ")}`);
        }
    ...
    async prompt(params) {
        ...
        session.promptPromise = new Promise((resolve) => {
            resolvePromptPromise = resolve;
        });
        try {
            const { stopReason, usage } = await this.runPromptLoop(params.sessionId, session, abortController.signal);
            ...
            if (abortController.signal.aborted) {
                ...
                if (userMessageId)
                    cancelled.userMessageId = userMessageId;
                return cancelled;
            }
            session.abortController = null;
    ...
    async unstable_forkSession(params) {
        ...
        return {
            ...
            configOptions: this.configOptionsState(forked.model, forked.thoughtLevel, forked.mode),
        };
    }
    ...
    persistSession(sessionId, session) {
        ...
        try {
            ...
            process.stderr.write(`[glm-acp-agent] warning: failed to persist session ${sessionId}: ${msg}\n`);
        }
    }
    requirePersisted(sessionId) {
        if (!this.sessionStore) {
     ...
     */
    async runPromptLoop(sessionId, session, signal) {
        const executor = new ToolExecutor(this.connection, sessionId, this.clientCapabilities, signal, this.visionClient, session.mcpTools, session.cwd, 
        // Use a thunk so mode changes mid-turn take effect on the next tool call.
        () => this.sessions.get(sessionId)?.mode ?? "default", (todos) => this.sessionTodos.set(sessionId, todos));
        let lastUsage;
        ...
        for (let turn = 0; turn < this.maxTurns; turn++) {
            ...
            try {
                ...
                const isOverflow = body?.code === ERR_CONTEXT_OVERFLOW ||
            ...
            for (const tc of toolCalls) {
                if (signal.aborted)
                    return { stopReason: "cancelled", usage: lastUsage };
    ...
    availableToolDefinitions(mcpTools = null) {
    ...
    for (const block of blocks) {
        switch (block.type) {
            ...
            case "audio":
                ...
                break;
            default:
                textParts.push(`[unknown block type ${block.type}]`);
        }
    }
 ...
 */
function availableModelsWith(currentModelId) {
    const available = getAvailableModels();
...
async function safeSessionUpdate(connection, params) {
    try {
