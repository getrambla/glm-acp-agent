 ...
 * include an image.
 */
export async function preprocessImageBlocks(blocks, visionClient, signal) {
 ...
 */
export function buildPromptBlockDiagnosticLines(blocks) {
...
function extractText(result) {
    ...
    if (!Array.isArray(content))
        return "";
    return content
        .map((entry) => (typeof entry === "object" && entry !== null && typeof entry.text === "string" ? entry.text : ""))
        .filter((s) => s.length > 0)
        .join("\n");
}
