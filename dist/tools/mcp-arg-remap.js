 ...
 */
export function remapArguments(requestedArgs, targetProperties) {
 ...
 */
export function resolveToolName(requestedName, availableTools, context) {
    if (!availableTools.length)
        return requestedName;
    if (availableTools.includes(requestedName))
        return requestedName;
    ...
    throw new Error(`Tool "${requestedName}" not available on ${context}. Available tools: [${availableTools.join(", ")}]`);
}
function extractToolKeywords(name) {
    const lower = name.toLowerCase();
    const keywords = [];
    if (lower.includes("search"))
        keywords.push("search");
    if (lower.includes("reader"))
        keywords.push("reader");
...
//# sourceMappingURL=mcp-arg-remap.js.map